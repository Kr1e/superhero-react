<?php
	 ...
	 */
	class Superhero_React_Rest_Controller {
		 ...
		 * @since    1.0.0
		 * @access   private
		 * @var      string    $route_namespace    The route namespace.
		 */
		private $route_namespace;

		/**
		 * The remote superhero API .
		 *
		 * @since    1.0.0
		 * @access   private
		 * @var      string    $remote_api    The route namespace.
		 */
		private $remote_api;


		/**
		 * Hardcoded superhero array.
		 *
		 * @since    1.0.0
		 * @access   private
		 * @var      array    $superhero_array    The route namespace.
		 */
		private $superhero_array;
		 ...
		 * @param      string    $route_namespace    The version of this plugin.
		 * @param      string    $remote_api    The remote superhero api url.
		 */
		public function __construct($plugin_name, $version, $route_namespace, $remote_api ) {

			$this->plugin_name = $plugin_name;
			$this->version = $version;
			$this->route_namespace = $route_namespace;
			$this->remote_api = $remote_api;
			$this->superhero_array = [
				['id' => 1, 'name' =>	'A-Bomb'],
		...
		public function register_routes() {
			register_rest_route($this->route_namespace, '/index', array(
				'methods' => 'GET',
				'callback' => array($this, 'index')
			));

			register_rest_route($this->route_namespace, '/superhero/(?P<superhero_id>\d+)', array(
				'methods' => 'GET',
				'callback' => array($this, 'getHeroById')
			));
		}

		/**
		 * Superhero index callback method
		 *
		 * @return \WP_REST_Response
		 */
		public function index($data)
		{
			$heroData = $this->getHeroIndex();

			return new \WP_REST_Response($heroData, 200);
		}

		/**
		 * Basic superhero data callback method
		 *
		 * @param $data
		 * @return \WP_REST_Response
		 */
		public function getHeroById($data)
		{
			$heroData = $this->getHeroDataHelper($data['superhero_id']);

			if( empty($heroData) ) return new \WP_REST_Response(array('response' => 'error'), 404);
		...
		/**
		 * Helper function that fetches superhero data by id. It cashes superhero data inside transient and serves from it if present.
		 *
		 * @param int $superhero_id
		 * @return object
		 */
		public function getHeroDataHelper($superhero_id)
		{
			$heroData = get_transient('hero_data_' . $superhero_id); // Check database - saves HTTP requests
			if( !empty($heroData) ){
				return $heroData;
			}
			$response = wp_remote_get($this->remote_api . '/' . $superhero_id );
			$heroData = wp_remote_retrieve_body($response);
			$heroData = json_decode($heroData);
			if( $heroData->response === 'error'){
				return array();
			} else {
				set_transient( 'hero_data_' . $superhero_id, $heroData, 12 * 60 * 60 ); // Store in database for up to 12 hours
				return $heroData;
			}
		}

		/**
		 * Helper function that fakes responde form the remote API on /index route since superheroeapi does not have /index route.
		 *
		 * @return array
		 */
		public function getHeroIndex()
		{
			return $this->superhero_array;
		}
